  ...
  const galleryItems = [
    {
      ...
      title: "Education Support Program",
      category: "Education",
      description: "Supporting students through learning resources, mentorship, and educational initiatives.",
      accent: "red",
    },
    {
      image: healthcareImg,
      title: "Community Healthcare Camp",
      category: "Healthcare",
      description: "Medical consultations, health screenings, and wellness awareness programs.",
      accent: "green",
    },
    {
      image: volunteerImg,
      title: "Volunteer Engagement Drive",
      category: "Volunteerism",
      description: "Volunteers coming together to support meaningful social initiatives.",
      accent: "blue",
    },
    {
      image: childImg,
      title: "Child Welfare Activities",
      category: "Child Welfare",
      description: "Programs focused on children's education, nutrition, and development.",
      accent: "red",
    },
    {
      image: elderlyImg,
      title: "Elderly Care Initiative",
      category: "Senior Welfare",
      description: "Supporting senior citizens through care, engagement, and wellness programs.",
      accent: "blue",
    },
    {
      image: environmentImg,
      title: "Environmental Awareness Campaign",
      category: "Environment",
      description: "Promoting sustainability through tree plantation and green initiatives.",
      accent: "green",
    },
    {
      image: foodImg,
    ...
    },
    {
      ...
      title: "CSR Partnership Activities",
      category: "CSR",
      description: "Collaborating with organizations to create sustainable social impact.",
      accent: "blue",
    },
    {
      image: skillImg,
      title: "Youth Skill Development Workshop",
      category: "Skill Development",
      description: "Empowering youth with career guidance and employability skills.",
      accent: "green",
    },
  ];

  return (
    <section className="gly-section" ref={sectionRef}>
      <div className="gly-container">
        
        {/* Header */}
        <motion.div
          className="gly-header"
          initial={{ opacity: 0, y: 30 }}
          animate={isInView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.7, ease: [0.33, 0.1, 0.25, 1] }}
        >
          <div className="gly-header-tag">
            <span className="gly-header-tag-line" />
            <span className="gly-header-tag-text">Our Gallery</span>
          </div>
          <h2 className="gly-header-title">
            Capturing Moments of <span className="gly-text-red">Impact</span>
          </h2>
          <p className="gly-header-desc">
            Explore the journey of Uvagai Foundation through our initiatives, 
            community programs, and social impact projects.
          </p>
        </motion.div>
        ...
        >
          {galleryItems.map((item, index) => (
            <motion.div
              className={`gly-card gly-card-${item.accent}`}
              key={index}
              variants={{ hidden: { opacity: 0, y: 25 }, visible: { opacity: 1, y: 0, transition: { duration: 0.5 } } }}
              whileHover={{ y: -5 }}
              onClick={() => setSelectedImage(item)}
            >
              <div className="gly-card-media">
                <img src={item.image} alt={item.title} loading="lazy" />
                <div className="gly-card-shade" />
                <div className="gly-card-badge">
                  <span className={`gly-card-dot gly-dot-${item.accent}`} />
                  <span className="gly-card-badge-text">{item.category}</span>
                </div>
                <div className="gly-card-view">
              ...
              <div className="gly-card-body">
                <h3 className="gly-card-title">{item.title}</h3>
                <p className="gly-card-text">{item.description}</p>
              </div>
              <div className={`gly-card-line gly-line-${item.accent}`} />
            </motion.div>
          ))}
        </motion.div>
        ...
        >
          <Link to="/gallerypage" className="gly-cta-btn">
        ...
        {selectedImage && (
          ...
          >
            <motion.div
              ...
              onClick={(e) => e.stopPropagation()}
            >
              <button className="gly-lightbox-close" onClick={() => setSelectedImage(null)}>
                <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
                  <path d="M5 5L17 17M17 5L5 17" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                </svg>
              </button>
              <div className="gly-lightbox-image">
                <img src={selectedImage.image} alt={selectedImage.title} />
              </div>
              <div className="gly-lightbox-info">
                <span className={`gly-lightbox-cat gly-cat-${selectedImage.accent}`}>{selectedImage.category}</span>
                <h3 className="gly-lightbox-title">{selectedImage.title}</h3>
                <p className="gly-lightbox-desc">{selectedImage.description}</p>
              </div>
            </motion.div>
          </motion.div>
        )}
