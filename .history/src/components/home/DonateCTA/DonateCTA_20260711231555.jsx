...
export default function Donate() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      setHeroCounts({ lives, communities, funds });
  ...
  // Typing effects
  useEffect(() => {
    const words = ["Together", "We", "Build", "Better", "Futures"];
    let i = 1;
    const timer = setInterval(() => {
      i++;
      if (i <= words.length) {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      setCounts({ transparency, lives, communities, years });
  ...
  // All donate actions now just navigate to the Contact page
              ...
              >
                ...
                <div className="slider-benefits">
                  {activeDonation.benefits.map((item, index) => (
                    <div key={index} className="slider-benefit">
                      ✓ {item}
                    </div>
                  ))}
                </div>
